...
inputs = varargin;
filtersize = checkparam('logsize', 13,inputs);
...
logstd = checkparam('logstd', 2,inputs);
dilatesize = checkparam('dilatesize', 31,inputs);
blendsize = checkparam('blendsize', 31,inputs);
blendstd = checkparam('blendstd', 5,inputs);
logthreshold = checkparam('logthreshold', 0,inputs);

function val = checkparam(param, defaultval,inputs)
    
    if any(strcmpi(inputs, param))
        val = inputs{find(strcmpi(inputs, param))+1};
    else
        val = defaultval;
    end
end
...
fmap = imgaussfilt(fmap,blendstd,'FilterSize',[blendsize blendsize]);
fmap(fmap < 1) = 1;
